...
public class Ewrapperimpl implements EWrapper {
	...
	private HashMap <Integer,ArrayList<Bar>> barsDict = new HashMap <>();
	private ArrayList<Contract> openOrderContracts = new ArrayList<>();
	private boolean isHistoricalDataRecived = false ;
	private boolean isAccountAvailableFundsRecived = false;
	//FOREX variabels only :
	private double currentTickBidPrice = 0.0;
	private double currentTickAskPrice = 0.0;
	private HashMap <Integer,Double> currentTickPrice = new HashMap<>() ;
	private Decimal currentOpenPositionSize = Decimal.get(0.0) ;
	private Contract currentOpenPositionContract ;
	private boolean isPositionsArived = false;
	private double currentOpenPositionCost =0.0;
	...
	public Double getAvailableFunds() {return this.accountAvailableFunds;}
	public HashMap <Integer,ArrayList<Bar>> getBarsDict() {return this.barsDict;}
	public Double getCurrentTickPrice (int reqId) {return this.currentTickPrice.get(reqId);}
	public boolean getIsHistoricalDataRecived() {return this.isHistoricalDataRecived;}
	public boolean getisAccountAvailableFundsRecived() {return this.isAccountAvailableFundsRecived;}
	public Decimal getCurrentOpenPositionSize() {return this.currentOpenPositionSize ;}
	public Contract getCurrentOpenPositionContract() {return this.currentOpenPositionContract;}
	public boolean getIsPositionsArived() {return this.isPositionsArived;}
	public double getCurrentOpenPositionCost() {return this.currentOpenPositionCost;}
	public String getBotMode() {return this.botMode;}
	public ArrayList<Contract> getOpenOrderContracts () {return this.openOrderContracts;}

	//SETTERS
	public void setAccountAvailableFunds(Double availableFunds) { this.accountAvailableFunds = availableFunds;}
	public void setIsHistoricalDataRecived(boolean isHistoricalDataRecived) {this.isHistoricalDataRecived = isHistoricalDataRecived;}
	public void setisAccountAvailableFundsRecived(boolean isAccountAvailableFundsRecived) {this.isAccountAvailableFundsRecived = isAccountAvailableFundsRecived;}
	public void setBotMode(String botMode) {this.botMode=botMode;}

	//METHODS
	public void resetOpenOrdersContracts () { openOrderContracts.clear();}
	public void resetBarsDict() { barsDict.clear();}
	public ArrayList<Double> getClosingPrices(int reqId) {
		if(this.barsDict.containsKey(reqId)) {
			ArrayList<Double> closingPrices = new ArrayList<>(Collections.nCopies(barsDict.get(reqId).size(),0.0));
			ArrayList<Bar> barsList = barsDict.get(reqId);
			for(int i=0; i<barsList.size(); i++) {
				closingPrices.set(i,barsList.get(i).close());
			}
	...
	public void tickPrice(int tickerId, int field, double price, TickAttrib attribs) {
		...
		if(!currentTickPrice.containsKey(tickerId)){
			currentTickPrice.put(tickerId,1.0);
		}

		if (this.botMode.equals("STK")||this.botMode.equals("FUT")||this.botMode.equals("CFD")) {
			if (field == 4) { // Last price for stocks
				this.currentTickPrice.put(tickerId,price) ;
			}
		} else if (this.botMode.equals("FX")) {
			if (field == 1) {
				this.currentTickBidPrice = price;
				if (currentTickAskPrice != 0) {
					this.currentTickPrice.put(tickerId,(currentTickBidPrice + currentTickAskPrice) / 2);
				}
			} else if (field == 2) {
				this.currentTickAskPrice = price;
				if (currentTickBidPrice != 0) {
					this.currentTickPrice.put(tickerId,(currentTickBidPrice + currentTickAskPrice) / 2);
				}
			}
		}
	...
	public void openOrder(int orderId, Contract contract, Order order, OrderState orderState) {
		...
		if (!openOrderContracts.contains(contract)) {
			openOrderContracts.add(contract);
		}
	}
	//! [openorder]
	...
	public void updateAccountValue(String key, String value, String currency, String accountName) {
		...
		if (key.equals("AvailableFunds")) {
			isAccountAvailableFundsRecived=true;
			setAccountAvailableFunds(Double.parseDouble(value));
		}
	}
	//! [updateaccountvalue]
...
//		System.out.println(EWrapperMsgGenerator.updateAccountTime( timeStamp));
	...
	public void historicalData(int reqId, Bar bar) {
		...
		if (!barsDict.containsKey(reqId)) {
			ArrayList<Bar> newBarList = new ArrayList<>();
			newBarList.add(bar);
			barsDict.put(reqId, newBarList);
		} else {
			barsDict.get(reqId).add(bar);
		}
	}
	//! [historicaldata]
...
//		System.out.println(EWrapperMsgGenerator.position(account, contract, pos, avgCost));
		currentOpenPositionSize = pos;
		currentOpenPositionContract = contract;
		currentOpenPositionCost = avgCost;
		System.out.println("!!!!!!! AVERAGE POSITION COST : "+avgCost);
		isPositionsArived=true;
	}
	//! [position]
